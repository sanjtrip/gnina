...
tree_gpu::tree_gpu(vector_mutable<ligand> &ligands,
  ...
  definitelyPinnedMemcpy(device_nodes, &nodes[0],
      sizeof(segment_node) * nodes.size(), hipMemcpyHostToDevice);

  device_malloc(&force_torques, sizeof(gfloat4p) * nodes.size());
  hipMemsetAsync(force_torques, 0, sizeof(gfloat4p) * nodes.size(),
      hipStreamPerThread);
...
void tree_gpu::deallocate(tree_gpu *t) {
  tree_gpu cpu;
  definitelyPinnedMemcpy(&cpu, t, sizeof(tree_gpu), hipMemcpyDeviceToHost);
