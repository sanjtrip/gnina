...
void evaluate_splines_host(const GPUSplineInfo& spInfo, float r,
  ...
  unsigned n = spInfo.n;
  hipLaunchKernelGGL(evaluate_splines, n, 1, 0, 0, (float**) spInfo.splines, r, spInfo.fraction,
...
template<typename T> T __host__ __device__  zero(void);

template<typename T> gfloat3 zero(void) {
  return gfloat3(0, 0, 0);
}

template<typename T> float zero(void) {
  return 0;
}
...
template<bool remainder> __host__  __device__
...
__host__  __device__ void reduce_energy(force_energy_tup *result, float energy) {
...
// ROCm-Port
//__host__  __device__ void reduce_energy(force_energy_tup *result, int n, float v,
...
float single_point_calc(const GPUNonCacheInfo &info, atom_params *ligs,
  ...
  if (nfull_blocks)
    hipLaunchKernelGGL(HIP_KERNEL_NAME(interaction_energy<0>), dim3(num_movable_atoms, nfull_blocks), THREADS_PER_BLOCK, 0, 0, info, 0, ligs, out);
  if (nthreads_remain)
    hipLaunchKernelGGL(HIP_KERNEL_NAME(interaction_energy<1>), num_movable_atoms, ROUND_TO_WARP(nthreads_remain), 0, 0, 
  ...
  hipLaunchKernelGGL(reduce_energy, 1, ROUND_TO_WARP(num_movable_atoms), 0, 0, out, num_movable_atoms,
...
#else
  float cpu_out;
  definitelyPinnedMemcpy(&cpu_out, &out->energy, sizeof(float), hipMemcpyDeviceToHost);
  return cpu_out;
#endif
}
...
void *getHostMem() {
  void *r = nullptr;
  CUDA_CHECK_GNINA(hipHostAlloc(&r, 40960 * 1024, hipHostMallocDefault));
  return r;
}

hipError_t definitelyPinnedMemcpy(void* dst, const void *src, size_t n,
    hipMemcpyKind k) {
  assert(k != hipMemcpyDefault);
  if (k == hipMemcpyDeviceToDevice || k == hipMemcpyHostToHost)
    return hipMemcpy(dst, src, n, k);
  thread_local void *buf = getHostMem();
  assert(n < 40960 * 1024);
  if (k == hipMemcpyHostToDevice) {
    memcpy(buf, src, n);
    CUDA_CHECK_GNINA(hipMemcpyAsync(dst, buf, n, k, hipStreamPerThread));
    CUDA_CHECK_GNINA(hipStreamSynchronize(hipStreamPerThread));
  } else {
    assert(k == hipMemcpyDeviceToHost);

    CUDA_CHECK_GNINA(hipMemcpyAsync(buf, src, n, k, hipStreamPerThread));
    CUDA_CHECK_GNINA(hipStreamSynchronize(hipStreamPerThread));
    memcpy(dst, buf, n);
  }
  return hipSuccess;
