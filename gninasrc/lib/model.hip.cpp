...
// ROCm-Port
  ...
  if (pairs_sz < CUDA_THREADS_PER_BLOCK) {
    hipLaunchKernelGGL(eval_intra_kernel, 1, pairs_sz, 0, 0, info.splineInfo, coords, pairs, pairs_sz,
        cutoff_sqr, v, minus_forces, scratch);

  } else {
    hipLaunchKernelGGL(eval_intra_kernel, CUDA_GET_BLOCKS(pairs_sz, 1024), CUDA_THREADS_PER_BLOCK, 0, 0, info.splineInfo, coords, pairs, pairs_sz,
        cutoff_sqr, v, minus_forces, scratch);
  }
#ifdef __CUDA_ARCH__ 
...
__device__ fl gpu_data::eval_deriv_gpu(const infoT& info, const vec& v,
  ...
  fl e, ie = 0;
  if (threadIdx.x == 0) {
    hipLaunchKernelGGL(set_conf_kernel, 1, treegpu->num_atoms, 0, 0, treegpu, atom_coords,
        (vec*) coords, c);
    ...
    e += ie;
    hipLaunchKernelGGL(derivatives_kernel, 1, treegpu->num_atoms, 0, 0, treegpu, (vec*) coords,
        (vec*) minus_forces, g);
