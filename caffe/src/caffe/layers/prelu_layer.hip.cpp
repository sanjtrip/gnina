...
void PReLULayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(PReLUForward<Dtype>), CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, 0, 
...
void PReLULayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0]) {
    ...
    // NOLINT_NEXT_LINE(whitespace/operators)
    hipLaunchKernelGGL(HIP_KERNEL_NAME(PReLUParamBackward<Dtype>), CAFFE_GET_BLOCKS(cdim), CAFFE_CUDA_NUM_THREADS, 0, 0, 
  ...
  if (propagate_down[0]) {
    ...
    // NOLINT_NEXT_LINE(whitespace/operators)
    hipLaunchKernelGGL(HIP_KERNEL_NAME(PReLUBackward<Dtype>), CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, 0, 
