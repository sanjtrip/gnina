...
void BNLLLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(BNLLForward<Dtype>), CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, 0, 
...
void BNLLLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    const int count = bottom[0]->count();
    // NOLINT_NEXT_LINE(whitespace/operators)
    hipLaunchKernelGGL(HIP_KERNEL_NAME(BNLLBackward<Dtype>), CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, 0, 
