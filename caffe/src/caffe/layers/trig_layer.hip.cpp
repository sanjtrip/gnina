...
void TrigLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if(func == TrigParameter_Function_COS)
    hipLaunchKernelGGL(HIP_KERNEL_NAME(CosForward<Dtype>), CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, 0, 
      count, bottom_data, top_data);
  else if(func == TrigParameter_Function_SIN)
    hipLaunchKernelGGL(HIP_KERNEL_NAME(SinForward<Dtype>), CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, 0, 
          count, bottom_data, top_data);
  else
...
void TrigLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    // NOLINT_NEXT_LINE(whitespace/operators)
    if(func == TrigParameter_Function_COS)
      hipLaunchKernelGGL(HIP_KERNEL_NAME(CosBackward<Dtype>), CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, 0, 
        count, top_diff, bottom_data, bottom_diff);
    else if(func == TrigParameter_Function_SIN)
      hipLaunchKernelGGL(HIP_KERNEL_NAME(SinBackward<Dtype>), CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, 0, 
        count, top_diff, bottom_data, bottom_diff);
    else
        CHECK(0) << "Unsupported trig function";
    CUDA_POST_KERNEL_CHECK;
