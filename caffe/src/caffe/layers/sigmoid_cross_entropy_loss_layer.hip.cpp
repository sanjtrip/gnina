...
void SigmoidCrossEntropyLossLayer<Dtype>::Forward_gpu(
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(SigmoidCrossEntropyLossForwardGPU<Dtype>), CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, 0, count, input_data, target, loss_data,
...
void SigmoidCrossEntropyLossLayer<Dtype>::Backward_gpu(
  ...
  if (propagate_down[0]) {
    ...
    if (has_ignore_label_) {
      // NOLINT_NEXT_LINE(whitespace/operators)
      hipLaunchKernelGGL(HIP_KERNEL_NAME(SigmoidCrossEntropyLossIgnoreDiffGPU<Dtype>), CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, 0, count, ignore_label_, target, bottom_diff);
    }
    // Scale down gradient
