...
void ELULayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(ELUForward<Dtype>), CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, 0, 
...
void ELULayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    Dtype alpha = this->layer_param_.elu_param().alpha();
    // NOLINT_NEXT_LINE(whitespace/operators)
    hipLaunchKernelGGL(HIP_KERNEL_NAME(ELUBackward<Dtype>), CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, 0, 
