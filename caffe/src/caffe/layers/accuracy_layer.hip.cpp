...
void AccuracyLayer<Dtype>::Forward_gpu(
  ...
  if (top.size() == 1) {
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(AccuracyForwardGPU<Dtype>), CAFFE_GET_BLOCKS(nthreads), CAFFE_CUDA_NUM_THREADS, 0, 0, nthreads, bottom_data, bottom_label,
    ...
    // NOLINT_NEXT_LINE(whitespace/operators)
    hipLaunchKernelGGL(HIP_KERNEL_NAME(AccuracyForwardWithPerClassGPU<Dtype>), CAFFE_GET_BLOCKS(nthreads), CAFFE_CUDA_NUM_THREADS, 0, 0, nthreads, bottom_data, bottom_label,
