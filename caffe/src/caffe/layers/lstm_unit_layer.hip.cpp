...
void LSTMUnitLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(LSTMActsForward<Dtype>), CAFFE_GET_BLOCKS(X_count), CAFFE_CUDA_NUM_THREADS, 0, 0, 
      X_count, hidden_dim_, X, X_acts);
  CUDA_POST_KERNEL_CHECK;
  // NOLINT_NEXT_LINE(whitespace/operators)
  hipLaunchKernelGGL(HIP_KERNEL_NAME(LSTMUnitForward<Dtype>), CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, 0, 
