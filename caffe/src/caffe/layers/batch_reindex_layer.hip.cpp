...
void BatchReindexLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(BRForward<Dtype>), CAFFE_GET_BLOCKS(threads), CAFFE_CUDA_NUM_THREADS, 0, 0, 
...
void BatchReindexLayer<Dtype>::Backward_gpu(
  ...
  // NOLINT_NEXT_LINE(whitespace/operators)
  hipLaunchKernelGGL(HIP_KERNEL_NAME(BRBackward<Dtype>), CAFFE_GET_BLOCKS(threads), CAFFE_CUDA_NUM_THREADS, 0, 0, 
