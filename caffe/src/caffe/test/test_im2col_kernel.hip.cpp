...
TYPED_TEST(Im2colKernelTest, Test2D) {
  ...
  for (int grid_div = 2; grid_div <= 8; grid_div++) {
    for (int n = 0; n < this->blob_bottom_->num(); ++n) {
      ...
      hipLaunchKernelGGL(HIP_KERNEL_NAME(im2col_gpu_kernel<TypeParam>), grid_dim, CAFFE_CUDA_NUM_THREADS, 0, 0, 
...
TYPED_TEST(Im2colKernelTest, TestND) {
  ...
  for (int grid_div = 2; grid_div <= 8; grid_div++) {
    for (int n = 0; n < this->blob_bottom_->num(); ++n) {
      ...
      // NOLINT_NEXT_LINE(whitespace/operators)
      hipLaunchKernelGGL(HIP_KERNEL_NAME(im2col_nd_gpu_kernel<TypeParam, 2>), grid_dim, CAFFE_CUDA_NUM_THREADS, 0, 0, 
